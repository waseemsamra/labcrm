...
const casesData = {
  'DP-88290': {
    ...
    due: 'Today, 4:00 PM',
    notes: 'Upper right molar (#3). Shade A2. Digital scan received 03/24. Precise margin fit required.',
    doctor: 'Dr. Bruce Wayne',
    tooth: '14, 15',
    material: 'Zirconia High Trans.',
    instruction: 'Full Contour Crown, Screw-Retained over Implant.',
  },
  'DP-88301': {
    patient: 'Sarah Davis',
    clinic: 'Metro Health',
    ...
    status: 'Pending',
    due: 'Oct 24, 2023',
    notes: 'Central Incisors (#8, #9). Bleach shade BL2. Minimal prep design confirmed.',
    doctor: 'Dr. Sarah Miller',
    tooth: '8, 9',
    material: 'IPS e.max Press',
    instruction: 'Minimal prep veneers, bleach shade BL2.',
  },
  'DP-88305': {
    ...
    status: 'In Review',
    due: 'Tomorrow',
    notes: '3-Unit bridge (#19-21). Titanium base. Verification jig requested by clinic.',
    doctor: 'Dr. James Wilson',
    tooth: '19, 20, 21',
    material: 'Titanium Grade 5',
    instruction: 'Screw-retained bridge with verification jig.',
  },
  'DP-88312': {
    patient: 'Karen Lee',
    ...
    status: 'In Progress',
    due: 'Oct 26, 2023',
    notes: 'Mandibular framework. Note on distal extension clasps from Dr. Smith.',
    doctor: 'Dr. Elena Costa',
    tooth: 'Full Arch',
    material: 'CoCr Framework',
    instruction: 'Partial denture with distal extension clasps.',
  },
}

export default function ClientCaseDetail() {
  const { caseId } = useParams()
  const caseData = casesData[caseId] || casesData['DP-88290']
      ...
      <header className="flex justify-between items-center mb-lg">
        <div>
          <nav className="flex items-center gap-xs text-on-surface-variant font-label-caps text-label-caps mb-xs">
            <Link to="/client/Cases" className="hover:text-primary">Cases</Link>
            <span className="material-symbols-outlined text-[14px]">chevron_right</span>
            <span className="text-primary">{caseId}</span>
          </nav>
          <div className="flex items-center gap-md">
            <h2 className="font-display-lg text-display-lg text-on-surface">Case: {caseData.patient}</h2>
            <span className={`px-sm py-xs rounded-full font-label-caps text-label-caps ${
              caseData.status === 'In Production' ? 'bg-secondary-container text-on-secondary-container' :
              caseData.status === 'Pending' ? 'bg-surface-container-highest text-tertiary' :
          ...
          <div className="bg-surface border border-outline-variant rounded p-md">
            ...
            <div className="space-y-md">
              <div>
                ...
                <p className="font-title-sm text-title-sm text-on-surface">{caseData.doctor}</p>
                <p className="font-body-sm text-body-sm text-on-surface-variant">{caseData.clinic}</p>
              </div>
              <div className="grid grid-cols-2 gap-md">
                <div>
                  <p className="text-[11px] text-on-surface-variant uppercase font-bold">Tooth #</p>
                  <p className="font-body-md text-body-md text-on-surface">{caseData.tooth}</p>
                </div>
                <div>
                  <p className="text-[11px] text-on-surface-variant uppercase font-bold">Material</p>
                  <p className="font-body-md text-body-md text-on-surface">{caseData.material}</p>
                </div>
              </div>
              <div>
                <p className="text-[11px] text-on-surface-variant uppercase font-bold">Instruction</p>
                <p className="font-body-sm text-body-sm text-on-surface">{caseData.instruction}</p>
              </div>
            </div>
          </div>
